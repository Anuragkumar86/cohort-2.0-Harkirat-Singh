...
import { SignupInput} from '@anuragkumar86/medium-commons/dist/zod';
import { Link, useNavigate } from "react-router-dom";
import axios from "axios";
import { BACKEND_URL } from "../config";

const LeftPartSignup = () => {
  const [formData, setFormData] = useState<SignupInput>({name: "", email: "", password: "" });
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" name="name" placeholder="name" className="w-full p-3 border rounded mb-4 mt-2" onChange={handleChange} />
        ...
        <button onClick={sendRequest} type="submit" className="w-full bg-black text-white p-3 rounded">{"Sign up"}</button>
      </form>
    </div>
  );
};

const RightPartSignup = () => {
    return (
  ...
  export { LeftPartSignup, RightPartSignup };
